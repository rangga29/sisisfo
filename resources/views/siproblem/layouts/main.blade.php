...
<body class="home">
            ...
            <div class="container">
                <div class="d-flex flex-wrap align-items-center justify-content-center justify-content-lg-start">
                    ...
                    <ul class="nav col-12 col-lg-auto my-2 justify-content-center my-md-0 text-small">
                        <li>
                            <a href="{{ route('siproblem.home') }}" class="nav-link text-white">
                                <svg class="bi d-block mx-auto mb-1" width="24" height="24"><use xlink:href="#home"/></svg>
                                Dashboard
                            </a>
                        </li>
                        @auth
                            @if(auth()->user()->department->dp_spr)
                                <li>
                                    <a href="{{ route('siproblem.report.view-list') }}" class="nav-link text-white">
                                        <svg class="bi d-block mx-auto mb-1" width="24" height="24"><use xlink:href="#table"/></svg>
                                        SPR {{ auth()->user()->department->dp_name }}
                                    </a>
                                </li>
                            @endif
                        @endauth
                        <li>
                            <a href="{{ route('siproblem.reports.create') }}" class="nav-link text-white">
                        ...
                        @auth
                            @if(auth()->user()->role == 'Administrator')
                                <li>
                                    <a href="{{ route('siproblem.admin') }}" class="nav-link text-white">
                        ...
                        @endauth
                        <li>
                            <form action="{{ route('siproblem.auth.logout') }}" method="post">
                                @csrf
    ...
    <div class="col-lg-10 mx-auto p-4 py-md-5">
        ...
        <footer class="pt-2 my-5 text-body-secondary border-top">
    ...
    <script>
        $(document).ready(function() {
            ...
            let table1 = $('#requestTable').DataTable({
                "pageLength": 10
            });
            ...
            // Initialize second DataTable when its tab is shown
            $('#bagian-tab').on('shown.bs.tab', function () {
                if (!$.fn.DataTable.isDataTable('#requestTable2')) {
                    let table2 = $('#requestTable2').DataTable({
                        "pageLength": 10
                    });
        ...
        document.getElementById('add_dp_id').addEventListener('change', function() {
            const departmentId = this.value;
            const problemDropdown = document.getElementById('add_pr_id');
            const loadingOption = document.createElement('option');

            // Show loading state
            problemDropdown.innerHTML = '';
            loadingOption.textContent = 'Memuat data...';
            problemDropdown.appendChild(loadingOption);
            ...
            if (departmentId) {
                fetch(`/siproblem/reports/get-problems/${departmentId}`)
                    ...
                    .then(data => {
                        problemDropdown.innerHTML = '<option value="" selected disabled>Pilih Nama Kategori</option>';
                        ...
                        if (data.length === 0) {
                            ...
                            data.forEach(problem => {
                                ...
                                option.textContent = problem.pr_name;
                                problemDropdown.appendChild(option);
                            });
                        }
                    })
                    .catch(error => {
                        ...
                        problemDropdown.innerHTML = '<option value="" selected disabled>Gagal memuat data</option>';
                    })
                    .finally(() => {
